...
class PostgreSQL implements DatabaseInterface {
	...
	public function query($sql,$params=array()) {
		...
		/*
			...
			for twice :
					replace last occurence of ' with blank
		*/
		if (strpos($sql, '=ANY(') !== false) {
			$arr = explode('WHERE', $sql);
			$arrToReplace = $arr[1];
			$arrToReplace = str_replace('"', '', $arrToReplace);
			$arrToReplace = preg_replace('~(.*)' . preg_quote("'", '~') . '~', '$1' . "", $arrToReplace, 1);
			$arrToReplace = preg_replace('~(.*)' . preg_quote("'", '~') . '~', '$1' . "", $arrToReplace, 1);
			$sql = $arr[0]."WHERE".$arrToReplace;
			// var_dump($arr[0]."WHERE".$arrToReplace);
		}
		...
		if ($comparator[0]!='n') {
			if (strlen($comparator)==2) {
				switch ($comparator) {
					...
					case 'ac': return array('\'!\'=ANY(?)',$this->db->likeEscape($value),$field);
